"""
CP1404/CP5632 Practical
Demos of various os module examples

Moving renamed files to temp folder
while keeping original files in original folder
"""
import shutil
import os


def main():
    """Demo of os module functions."""
    print("Starting directory is: {}".format(os.getcwd()))

    # Change to desired directory
    os.chdir('Lyrics/Christmas')

    # Print a list of all files in current directory
    print("Files in {}:\n{}\n".format(os.getcwd(), os.listdir('.')))

    # Make a new directory
    # The next time you run this, it will crash if the directory exists
    # DONE: Use exception handling to avoid the crash (just pass)

    try:
        os.mkdir('temp')
    except FileExistsError:
        pass

    # Loop through each file in the (current) directory
    for filename in os.listdir('.'):
        # Ignore directories, just process files
        if os.path.isdir(filename):
            continue

        new_name = get_fixed_filename(filename)
        print("Renaming {} to {}".format(filename, new_name))

        # Done: Try these options one at a time
        # Option 1: rename file to new name - in place
        # os.rename(filename, new_name)

        # Option 2: move file to new place, with new name
        shutil.copy(filename, 'temp/' + new_name)

        # Process all subdirectories using os.walk()
    os.chdir('..')  # '..' means to go 'up' one directory
    lyrics_path = os.getcwd()  # store the path so we can get back to it
    for directory_name, subdirectories, file_names in os.walk('.'):
        print("Directory:", directory_name)
        os.chdir(directory_name)
        if directory_name != ".":
            if "temp" not in directory_name:
                try:
                    os.mkdir('temp')
                except FileExistsError:
                    pass
        print("\tcontains subdirectories:", subdirectories)
        print("\tand files:", file_names)
        print("(Current working directory is: {})".format(os.getcwd()))
        print(directory_name)

        if "temp" not in directory_name:
            for filename in file_names:
                # Ignore directories, just process files
                if os.path.isdir(filename):
                    continue

                new_name = get_fixed_filename(filename)
                print("Renaming {} to {}".format(filename, new_name))

                shutil.copy(filename, 'temp/' + new_name)
        os.chdir(lyrics_path)
        # Done: change into the directory and print the current working directory
        # then change back to the lyrics_path
        # Note: if you get this wrong, walk will stop short,
        # so you need to check it still walks through all subdirectories

        # Done: add a loop (in between directory changes) to rename the files


def get_fixed_filename(filename):
    """Return a 'fixed' version of filename."""
    new_name = filename.replace(" ", "_").replace(".TXT", ".txt")
    temp_name = ""
    for i in range(0, len(new_name)):
        if i == 0:
            temp_name += new_name[i].upper()
        elif i == len(new_name) - 1:
            temp_name += new_name[i]
        elif new_name[i - 1] == "_":
            temp_name += new_name[i].upper()
        elif new_name[i - 1] == "(":
            temp_name += new_name[i].upper()
        elif new_name[i] == "(":
            if new_name[i - 1] == "_":
                temp_name += new_name[i]
            else:
                temp_name += "_"
                temp_name += new_name[i]
        elif new_name[i].isupper():
            if new_name[i - 1] == "_":
                temp_name += new_name[i].upper()
            elif new_name[i - 1] == "(":
                temp_name += new_name[i].upper()
            else:
                temp_name += "_"
                temp_name += new_name[i].upper()
        else:
            temp_name += new_name[i]
    return temp_name


main()
